...
interface CarbonInterface extends DateTimeInterface, JsonSerializable
{
     ...
     * (second-precision).
	 * 将当前实例修改为给定实例的平均值(默认值)和当前实例(第二精度)。
     *
	 ...
	 * 返回通过的碳实例,现在在同一时区，如果给定的字符串为空给定或解析输入。
     *
	 ...
	 * 从timestamp创建一个碳实例并设置时区(如果不指定,使用默认一个)。
     *
	 ...
	 * 将日期重新设置为世纪末和时间,至23:59:59.999999
     *
     * @example
     * ```
	 ...
	 * 从一个本地化的字符串中创建一个碳实例(在法语、日语、阿拉伯语等)。
     *
