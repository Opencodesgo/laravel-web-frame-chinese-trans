...
class CarbonInterval extends DateInterval implements CarbonConverterInterface
{
	 ...
	 * 如果设置，则返回给定源到目标对的因子，
	 * 否则尝试找到适当的常数作为因子，例如Carbon::DAYS_PER_WEEK。
     *
     * @param string $source
     * @param string $target
     *
	 ...
	 * 根据指定的格式将字符串解析为新的CarbonInterval对象
     *
     * @example
     * ```
	 ...
	 * 提供静态帮助来创建实例。(3)允许CarbonInterval:年。
     *
	 ...
	 * 计算由var_export（）生成的PHP并重新创建导出的CarbonInterval实例。
     *
	 ...
	 * 使用不同的语言环境从字符串创建一个CarbonInterval
     *
	 ...
	 * 如果可能的话，从给定的变量创建一个CarbonInterval实例。
     *
	 ...
	 * 允许setter调用流畅…CarbonInterval:年(3)- >()个月(5)- >的一天。
     *
     * Note: This is done using the magic method to allow static and instance methods to
     *       have the same names.
     *
	 ...
	 * 返回与当前实例匹配的本机DateInterval PHP对象。
     *
     * @example
     * ```
